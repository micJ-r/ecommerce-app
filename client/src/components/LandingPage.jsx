  ...
  const retryFetch = () => {
    ...
    (async () => {
      try {
        ...
        else throw new Error('Products data not found in response');
      } catch (err) {
        setError(err.message);
      } finally {
        setIsLoading(false);
        ...
        <div className="container mx-auto flex flex-col md:flex-row items-center">
          ...
          <div className="md:w-1/2 mb-8 md:mb-0 md:pr-8 z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="bg-white text-amber-700 px-6 py-3 rounded-lg font-bold hover:bg-amber-100 transition-colors flex items-center justify-center">
                Shop Now <FiArrowRight className="ml-2" />
              </button>
              <button className="bg-transparent border-2 border-white px-6 py-3 rounded-lg font-bold hover:bg-white hover:text-amber-700 transition-colors">
                Learn More
              </button>
            </div>
          </div>
          ...
          <div className="md:w-1/2 relative z-10">
            ...
            />
            {/* Decorative elements */}
            <div className="absolute -bottom-4 -right-4 w-24 h-24 bg-amber-300 rounded-full opacity-20"></div>
